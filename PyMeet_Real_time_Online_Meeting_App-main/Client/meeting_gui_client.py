"""
PyMeet - GUI Wrapper (modular views)
...
class MeetingApp(tk.Tk):
    ...
    def _handle_message(self, msg: dict) -> None:
        ...
        if t == "login_ok":
            if "LoginView" in self.views and hasattr(self.views["LoginView"], "set_status"):
                self.views["LoginView"].set_status("Login successful!", ok=True)
            self.show("LobbyView")
    ...
    def do_login(self, username: str, email: str) -> None:
        if self.client.sock is None:
            try:
                ...
                if "LoginView" in self.views and hasattr(self.views["LoginView"], "set_status"):
                    self.views["LoginView"].set_status(f"Could not connect to server: {e}", ok=False)
                return
        self.username = username
        self.client.send({"type": "login", "payload": {"username": username, "password": email or "nopass"}})
        if "LoginView" in self.views and hasattr(self.views["LoginView"], "set_status"):
            self.views["LoginView"].set_status("Logging in…", ok=None)
    ...
    def toggle_mic(self) -> bool:
        try:
            ...
            messagebox.showwarning("Audio", "PyAudio or voice client not installed.")
            return False
        if not self.room:
            messagebox.showwarning("Audio", "Please join a room first.")
            return False
        if not self.mic_on:
    ...
    def toggle_cam(self, show_window: bool = False) -> bool:
        try:
            ...
            messagebox.showwarning("Video", "OpenCV or video client not installed.")
            return False
        if not self.room:
            messagebox.showwarning("Video", "Please join a room first.")
            return False
        if not self.cam_on:
