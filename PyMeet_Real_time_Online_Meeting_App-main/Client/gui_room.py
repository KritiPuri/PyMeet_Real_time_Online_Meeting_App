...
class RoomView(ttk.Frame):
    def __init__(self, parent, app):
        ...
        ttk.Button(self.hdr, text="Leave room", style="Danger.TButton",
        ...
        # Participants
        left = ttk.Labelframe(body, text="Participants", style="Card.TLabelframe", padding=10)
        ...
        self.btn_mic = ttk.Button(ctrls, text="🎙  Mic On", command=self._toggle_mic)
        self.btn_cam = ttk.Button(ctrls, text="🎥  Cam On", command=self._toggle_cam)
        self.btn_mic.pack(side=tk.LEFT)
        self.btn_cam.pack(side=tk.LEFT, padx=6)

        # Chat
        right = ttk.Labelframe(body, text="Messages", style="Card.TLabelframe", padding=10)
        ...
        ttk.Button(row, text="Send", style="Primary.TButton",
    ...
    def _toggle_cam(self) -> None:
        if not self.vclient:
            ...
            host = getattr(self.app.client, "host", "127.0.0.1") if hasattr(self.app, "client") else "127.0.0.1"
        ...
        if self.cam_visible:
            self.btn_cam.configure(text="🎥  Cam Off", style="Success.TButton")
        else:
            self.canvas.delete("local")
            self.btn_cam.configure(text="🎥  Cam On", style="TButton")

    # ------------------- Mic toggle -------------------
    def _toggle_mic(self) -> None:
        on = self.app.toggle_mic()
        self.btn_mic.configure(
            text="🎙  Mic Off" if on else "🎙  Mic On",
            style="Success.TButton" if on else "TButton"
        )

    # ------------------- Chat & room info -------------------
    def _title_text(self) -> str:
        return f"{self._room_name}  •  {self._count} users"

    def set_room(self, name: str) -> None:
        self._room_name = f"Room: {name}"
    ...
    def user_joined(self, who: str) -> None:
        ...
        self._count += 1
        self.lbl_title.configure(text=self._title_text())
        self.append_chat(f"* {who} joined the room *")
    ...
    def user_left(self, who: str) -> None:
        ...
        for name in items:
            if name != who:
                ...
                self._count += 1
        self.lbl_title.configure(text=self._title_text())
        self.append_chat(f"* {who} left the room *")
        if hasattr(self, '_remote_frames') and who in self._remote_frames:
