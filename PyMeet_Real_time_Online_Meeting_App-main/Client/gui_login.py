"""
LoginView (updated) - login screen for PyMeet.
- Username: only letters/numbers/._- (3-20 characters), no spaces.
- Email: valid format; automatically converted to lowercase.
- Continue button auto locks/unlocks based on valid state; Enter to submit.
- Loads PyMeet logo if Client/assets/pymeet_logo.png exists.
"""
import os
import re
...
class ValidEntry(ttk.Frame):
    """Entry with realtime validation + status icon + callback when validity changes."""
...
class LoginView(ttk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        ttk.Label(top, text="PyMeet", font=FONT_TITLE).pack(side=tk.LEFT)
        ttk.Label(card, text="Quick login, no registration required", style="Muted.TLabel").pack(anchor=tk.W, pady=(6, 12))

        self.btn_ok = ttk.Button(card, text="Login", style="Primary.TButton", command=self._submit)
        # entries
        self.ent_user = ValidEntry(card, "Username",
                                   pattern=self.USER_PAT,
                                   on_valid_change=self._sync_btn)
        self.ent_user.pack(fill=tk.X)

        self.ent_mail = ValidEntry(card, "Email address",
    ...
    def _load_logo(self):
        import os
        paths = (
            os.path.join(ASSETS_DIR, "pymeet_logo.png"),
            os.path.join(os.path.dirname(__file__), "pymeet_logo.png"),
        )
        for p in paths:
            if not os.path.exists(p):
                continue
            try:
                # Prefer Pillow for smooth resizing
                try:
                    ...
                    # Fallback without Pillow: use PhotoImage subsample
    ...
    def _submit(self) -> None:
        if not (self.ent_user.valid() and self.ent_mail.valid()):
            self.set_status("Please enter a valid username and email.", ok=False)
            self._sync_btn()
            return
        # chuẩn hóa email
