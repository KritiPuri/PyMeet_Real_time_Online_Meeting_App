...
class VideoCallClient:
    ...
    def _open_camera(self):
        # Try DirectShow on Windows first for better performance
        cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
        if not cap or not cap.isOpened():
            # Fallback to default
            cap = cv2.VideoCapture(0)
        if not cap or not cap.isOpened():
            raise RuntimeError("Cannot open camera. Please check if your camera is connected and not in use by another application.")
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 360)
        cap.set(cv2.CAP_PROP_FPS, 15)  # Limit to 15 FPS for better network performance
        return cap
    ...
    def _tx_loop(self) -> None:
        ...
        frame_delay = 1.0 / 15  # Target 15 FPS
        last_frame_time = 0
        while self._alive:
            try:
                # Frame rate limiting
                ...
                # if cam OFF -> don't send frames, only send keepalive
                ...
                if not self._alive:
                    break
                print(f"[VideoCall] Connection error in _tx_loop: {e}")
                time.sleep(0.5)
            except Exception:
                if self._alive:
