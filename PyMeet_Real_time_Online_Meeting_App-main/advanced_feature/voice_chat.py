...
class VoiceChatClient:
    ...
    - start(room, user): open mic/speaker and JOIN.
    - stop(): close stream, LEAVE.
    - mute: set self.mic_enabled = False to send silence frames.
    - volume_playback: 0.0..2.0 (multiply amplitude during playback).
    """
    def __init__(self,
                 host: str,
                 port: int,
        ...
        self.mic_enabled = True         # toggle mic (but keep connection)
        self.volume_playback = 1.0       # multiply amplitude during playback
    ...
    def start(self, room: str, user: str) -> None:
        ...
        try:
            ...
            # No mic? Still run but send silence
            self._mic = None
            ...
            try:
                if self.mic_enabled and self._mic:
                    try:
                        ...
                        # read error -> send silence
                        frame = b"\x00" * FRAME_BYTES
                else:
                    # mic off -> send silence to maintain sync
                    frame = b"\x00" * FRAME_BYTES
                ...
                if not self._alive:
                    break
                if self.on_error:
                    self.on_error(f"TX connection error: {e}")
                time.sleep(0.5)
            except Exception as e:
                if self.on_error and self._alive:
    ...
    def _rx_loop(self) -> None:
        ...
        while self._alive:
            try:
                ...
                # Windows-specific error when remote host closes connection
                if not self._alive:
                    break
                time.sleep(0.1)
                continue
