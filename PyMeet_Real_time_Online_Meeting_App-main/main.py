...
logger = logging.getLogger("PyMeet-Main")
...
class PyMeetSystem:
    ...
    async def start_all(self):
        logger.info("=" * 56)
        logger.info("PyMeet System Starting (ALL)...")
        logger.info("=" * 56)
    ...
    async def start_servers(self):
        logger.info("=" * 56)
        logger.info("PyMeet System Starting (SERVERS: TCP+UDP)...")
        logger.info("=" * 56)
        logger.info(f"TCP Server:    {self.server_host}:{self.tcp_port}")
        logger.info(f"UDP Server:    {self.server_host}:{self.udp_port}")
        logger.info("=" * 56)
    ...
    async def shutdown(self):
        if not self.running:
            return
        logger.info("Shutting down PyMeet System...")
        self.running = False
        ...
        logger.info("PyMeet System stopped")


def setup_signal_handlers(system: PyMeetSystem):
...
async def main():
    parser = argparse.ArgumentParser(description="PyMeet System")
    parser.add_argument(
        "--component",
    ...
    system = PyMeetSystem(args.host, args.tcp_port, args.udp_port, args.gateway_port)
